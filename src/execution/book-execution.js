...
class Book {
  ...
  apply(event,ts){
    ...
    if(event.type==='orderbook_snapshot'){
      ...
      const sides = [m.yes_dollars_fp, m.no_dollars_fp].map(rows => {
        const book = new Map();
        for (const row of rows) {
          if (!Array.isArray(row) || row.length !== 2) return null;
          const [p, n] = row.map(Number);
          if (!Number.isFinite(p) || p <= 0 || p >= 1 || !Number.isFinite(n) || n < 0) return null;
          book.set(p, n);
        }
        return book;
      });
      if (sides.some(side => side === null)) { this.valid=false; return; }
      [this.yes, this.no] = sides; this.valid=true; this.ts=ts;
      ...
      if(!book||!Number.isFinite(price)||price<=0||price>=1||!Number.isFinite(delta)){this.valid=false;return;}
      const quantity=(book.get(price)||0)+delta;
      if(!Number.isFinite(quantity)||quantity<0){this.valid=false;return;}if(quantity===0)book.delete(price);else book.set(price,quantity);this.ts=ts;
    }
    // Snapshots validate every level; a delta can only invalidate its touched level.
  }
