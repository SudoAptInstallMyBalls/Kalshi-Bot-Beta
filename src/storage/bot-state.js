...
const PERSIST_DIR = process.env.BOT_DATA_DIR ? path.resolve(process.env.BOT_DATA_DIR) : require('#src/config/paths').dataDir;
...
class BotState extends EventEmitter {
  ...
  _loadState() {
    try {
      ...
      if (saved.riskState) this.riskState = saved.riskState;
  ...
  _writeToDisk(throwOnError = false) {
    try {
      if (this.stateLoadFailed) throw new Error('Refusing to overwrite unreadable persisted positions');
      ...
      this.safety?.halt('state_persistence_failed');
