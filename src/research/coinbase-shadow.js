...
function evaluateCoinbaseShadow(history, db, baselineRows, cutoff) {
  const ticks = db.prepare('SELECT * FROM proxy_ticks ORDER BY received_ms').all().sort((a, b) => availableAt(a) - availableAt(b));
