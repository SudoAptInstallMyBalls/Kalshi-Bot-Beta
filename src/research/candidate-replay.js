// Experimental snapshot of history-replay.js; frozen production/research baseline remains untouched.
...
async function replay(history, spotRows, config, { modelPath, adverse = false, tickers = null } = {}) {
  ...
  const settlementReference = config.strategy.SETTLEMENT_AWARE
    ? new (require('./candidate-reference').CandidateReference)(history.prepare('SELECT * FROM markets ORDER BY close_time,ticker').all(), spotRows, { volatilityReturns: config.strategy.RESEARCH_VOLATILITY_RETURNS ?? 15, quantile: config.strategy.RESEARCH_BASIS_QUANTILE ?? .95 })
    : undefined;
