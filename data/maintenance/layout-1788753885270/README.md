...
> Current runtime: `server.js` → `MasterAgent` → skills, one persistent local Node process.
> Analytics and ML features use SQLite (`data/analytics.db`); trained models use
> `data/models/current-model.json`. The older runnable bot generations have been removed.
> Run `npm test` for offline regression checks and `npm run benchmark:ml` for timing.
> See [implementation notes](IMPLEMENTATION_NOTES.md) for current safeguards and
> [demo burn-in](DEMO_BURN_IN.md) before starting the changed bot.
> The strategy descriptions and parameter tables below are historical documentation;
> `server.js` and `.env.example` define the current defaults. In particular, the active
> generator does not produce dual-side arbitrage entries, and the scan interval is 2 seconds.
