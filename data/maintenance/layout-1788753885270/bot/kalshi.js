...
const fs = require('fs');
const { normalizeOrder } = require('../lib/kalshi-order');
const { notSubmitted } = require('../lib/order-rejection');
const { orderCost } = require('../lib/trading-math');
...
class KalshiClient {
  ...
  async fetchBalance() {
    try {
      ...
      if(this.config.ENABLE_TELEMETRY) require('../lib/research-telemetry').record('recordEquity',balance,this.state.openPositions || [],this.state.riskState);
...
async fetchMarket(ticker) {
  ...
  async placeOrder(orderData) {
    ...
    try {
      if(this.config.ENABLE_TELEMETRY) require('../lib/research-telemetry').record('recordEvent','submit',{...routed});
      const resp = await this.post('/trade-api/v2/portfolio/events/orders', routed);
      if(this.config.ENABLE_TELEMETRY) require('../lib/research-telemetry').record('recordEvent','acknowledgment',{client_order_id:routed.client_order_id,order:resp.data.order || resp.data});
      this.state.safety?.executionSucceeded();
      return normalizeOrder(resp.data.order || resp.data);
    } catch (err) {
      this.state.safety?.executionFailed();
      if(this.config.ENABLE_TELEMETRY) require('../lib/research-telemetry').record('recordEvent','submission_error',{client_order_id:routed.client_order_id,status:err.response?.status,code:err.response?.data?.error?.code});
      throw err;
    }
  }
  ...
  async getOrder(orderId) {
    ...
    if(this.config.ENABLE_TELEMETRY) require('../lib/research-telemetry').record('recordEvent','order_observed',{order:resp.data.order || resp.data});
    return normalizeOrder(resp.data.order || resp.data);
  }
  ...
  async cancelOrder(orderId, ticker) {
    ...
    if(this.config.ENABLE_TELEMETRY) require('../lib/research-telemetry').record('recordEvent','cancel_request',{orderId,ticker});
    ...
    if(this.config.ENABLE_TELEMETRY) require('../lib/research-telemetry').record('recordEvent','cancel_acknowledgment',{orderId,ticker,response:resp.data});
    return resp.data;
  }
