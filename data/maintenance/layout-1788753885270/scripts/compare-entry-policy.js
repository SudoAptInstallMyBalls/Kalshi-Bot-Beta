...
async function main() {
  ...
  try {
    const base = JSON.parse(fs.readFileSync(path.join(root, 'research-config.json')));
    ...
    const manifest = { base, variants, markets, startingBalance: 59.4,
      sourceHash: crypto.createHash('sha256').update(fs.readFileSync(path.join(root, 'agents/skills/analysis/signal-generator.js'))).digest('hex'),
