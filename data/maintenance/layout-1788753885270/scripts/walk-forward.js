...
async function main(){
  ...
  try {
    ...
    const policyPath=path.resolve(root,args[1]||'research-policy.json');
    ...
    const manifest={config,policy,policyPath,marketIds:markets.map(m=>m.ticker),schemes:'anchored; initial 40%, step/test 20%; inner last 25% of earlier markets for selection',
      ...
      hashes:Object.fromEntries(['lib/history-replay.js','lib/ml-pipeline.js','lib/regularized-logistic.js','agents/skills/analysis/signal-generator.js'].map(f=>[f,crypto.createHash('sha256').update(fs.readFileSync(path.join(root,f))).digest('hex')]))};
