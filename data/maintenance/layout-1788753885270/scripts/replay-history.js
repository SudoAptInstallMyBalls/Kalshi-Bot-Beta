...
const { openResearch, downloadSpot } = require('../lib/research-data');
const { replay, validateConfig } = require('../lib/history-replay');
...
function parseArgs(args) {
  const options = { history: path.join(root, 'data/market-history/history.sqlite'),
    out: path.join(root, 'data/research'), config: path.join(root, 'research-config.json'), download: false };
...
async function main(args = process.argv.slice(2)) {
  ...
  try {
    ...
    const report = { id, createdAt: new Date().toISOString(), platform: `${process.platform}/${process.arch}`,
      ...
      sourceHashes: Object.fromEntries(['lib/history-replay.js', 'lib/ml-pipeline.js',
        'agents/skills/analysis/signal-generator.js', 'agents/skills/analysis/probability-model.js'].map(file =>
