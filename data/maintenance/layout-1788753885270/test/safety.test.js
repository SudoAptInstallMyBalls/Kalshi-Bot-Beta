...
const TradingSafety = require('../lib/trading-safety');
const RiskManager = require('../agents/skills/trading/risk-manager');
const KalshiClient = require('../bot/kalshi');
const OrderExecutor = require('../agents/skills/trading/order-executor');
const MasterAgent = require('../agents/core/master-agent');
...
test('fifth failed placement stops sixth POST; a successful placement resets the streak', async t => {
  ...
  fail = false;
  await client.placeOrder({ reduce_only: true }); // Existing positions can still exit.
  assert.equal(state.safety.haltReason, 'execution_failures');
});
