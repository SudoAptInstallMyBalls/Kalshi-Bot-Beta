...
async function downloadSpot(db, from, to, { request = axios.get, pause = sleep, log = console.log } = {}) {
  ...
  for (let cursor = start; cursor < end; cursor += 1000 * 60000) {
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        response = await request('https://data-api.binance.vision/api/v3/klines', {
          params: { symbol: 'BTCUSDT', interval: '1m', startTime: cursor, endTime: stop - 1, limit: 1000 },
          timeout: 30000,
        });
        break;
      } catch (error) {
    ...
    db.transaction(() => {
      for (const raw of response.data) {
        ...
        insert.run(row.open, row.available, row.close, 'binance:BTCUSDT:1m', row.raw);
        downloaded++;
      }
    })();
    log(`[Reference] ${downloaded} BTC candles downloaded`);
    await pause(250);
  }
  const stored = count.get(start, end).n, expected = (end - start) / 60000;
  if (stored !== expected) throw new Error(`Incomplete BTC coverage: ${stored}/${expected} minutes. Rerun to resume.`);
