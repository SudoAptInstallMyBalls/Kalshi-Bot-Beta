...
function executionTotals(order) {
  ...
  if (!Number.isFinite(filled) || filled < 0) throw new Error('Order fill count unavailable');
  if (!filled) return { filled, gross: 0, fees: 0 };
  if (o.taker_fill_cost == null && o.maker_fill_cost == null) throw new Error('Actual fill costs unavailable');
  if (o.taker_fees == null && o.maker_fees == null) throw new Error('Actual fill fees unavailable');
  const parts = ['taker_fill_cost', 'maker_fill_cost', 'taker_fees', 'maker_fees'].map(k => Number(o[k] ?? 0));
  if (!parts.every(v => Number.isFinite(v) && v >= 0)) throw new Error('Invalid execution amounts');
  return { filled, gross: parts[0] + parts[1], fees: parts[2] + parts[3] };
}
module.exports = { normalizeOrder, executionTotals };
