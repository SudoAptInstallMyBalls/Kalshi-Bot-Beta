...
const { takerFee, orderCost, affordableContracts } = require('../../../lib/trading-math');
      ...
      case 'generate-signals': {
        ...
        if (this.telemetryEnabled) {
          ...
          if (!this.lastEntrySummary || Date.now() - this.lastEntrySummary >= 30000) {
            require('../../../lib/research-telemetry').record('recordEvent', 'entry_filter_summary', {
              counts: this.entryCounters, intervalStart: this.lastEntrySummary || Date.now(),
              thresholds: { minDivergence: this.minDivergence, minNetEdge: this.minNetEdge, windowMinutes: this.tradingWindow / 60000 },
            });
            this.entryCounters = {};
...
_getPortfolioExposure(state) {
  ...
  _calculatePositionSize({ state, ticker, price, edge, probability, probModel, committedThisPass = 0, committedForTicker = 0 }) {
    ...
    if(contracts<1 && this.telemetryEnabled) {
      ...
      require('../../../lib/research-telemetry').record('recordEvent','sizing_rejection',{ticker,price,positionDollars,reason});
  ...
  _generateSignals(kalshiMarkets, state, now = Date.now(), diagnostics = null) {
    ...
    for (const market of kalshiMarkets) {
      ...
      if (timeSinceOpen < 0 || timeSinceOpen > this.tradingWindow || timeRemaining < 30000) { count('outside_entry_window'); continue; }
      ...
      if (this.telemetryEnabled) require('../../../lib/research-telemetry').record('recordForecast', {
      ...
      if(this.telemetryEnabled) {
        ...
        require('../../../lib/research-telemetry').record('recordEvent','forecast_context', {
  ...
  _generateExitSignals(openPositions, kalshiMarkets, now = Date.now()) {
    ...
    for (const pos of openPositions) {
      ...
      // ----------------------------------------------------
      const pastEmergencyGrace = holdTime >= this.emergencyStopGraceMs;
      const emergencyDollarStop = pastEmergencyGrace && unrealizedPnL <= -Math.abs(this.maxLossPerPosition);
      ...
      // ----------------------------------------------------
      const pastGracePeriod = holdTime >= this.stopLossGracePeriodMs;
      const allowedBeforeSettlement = timeRemaining > this.stopLossMinTimeRemaining;
      const percentageStop = lossPct >= this.stopLossPct;
      ...
      // ----------------------------------------------------
      if (this.enableScalping && profitPct >= this.scalpQuickProfitPct) {
        signals.push({
      ...
      // ----------------------------------------------------
      const pastMinHold = holdTime >= this.scalpMinHoldMs;
      if (this.enableScalping && pastMinHold && profitPct >= this.scalpTakeProfitPct) {
        signals.push({
          type: 'SCALP_TAKE_PROFIT', orderId: pos.orderId, ticker: pos.ticker, side: pos.side,
          sellPriceCents: Math.round(currentValue * 100), sellPriceDecimal: currentValue,
          contracts, profitPct, unrealizedPnL,
      ...
      if (profitPct >= this.takeProfitPct || gainFraction >= this.takeProfitGainFraction) {
        signals.push({
          type: 'TAKE_PROFIT', orderId: pos.orderId, ticker: pos.ticker, side: pos.side,
          sellPriceCents: Math.round(currentValue * 100), sellPriceDecimal: currentValue,
          contracts, profitPct, unrealizedPnL,
