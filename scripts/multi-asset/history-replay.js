...
function spotContext(rows, index, strategy) {
  ...
  const fastK = 1 - (1 - 2 / ((strategy.TREND_FAST_PERIOD ?? DEFAULTS.TREND_FAST_PERIOD) + 1)) ** 60;
  const slowK = 1 - (1 - 2 / ((strategy.TREND_SLOW_PERIOD ?? DEFAULTS.TREND_SLOW_PERIOD) + 1)) ** 60;
  ...
  const roc = (last / past - 1) * 100, threshold = strategy.TREND_ROC_THRESHOLD ?? DEFAULTS.TREND_ROC_THRESHOLD;
  const trend = fast > slow && roc > threshold ? 'BULLISH' : fast < slow && roc < -threshold ? 'BEARISH' : 'NEUTRAL';
  const returnCount = strategy.RESEARCH_VOLATILITY_RETURNS ?? 15;
  const sigma = minuteVolatility(slice, 900, returnCount);
...
async function replay(history, spotRows, config, { asset = 'BTC', modelPath, adverse = false, tickers = null } = {}) {
  validateConfig(config);
  if (!['BTC','ETH','SOL','XRP','DOGE'].includes(asset) || spotRows.some(r=>r.source !== `coinbase:${asset}-USD:1m`)) throw Error('Mixed or unsupported asset');
  const markets = history.prepare("SELECT * FROM markets WHERE result IN ('yes','no') ORDER BY open_time,ticker").all()
    .filter(m => m.ticker.startsWith(`KX${asset}15M-`) && (!tickers || tickers.has(m.ticker)));
  ...
  let currentSpot, currentState;
  const registry = require('../../src/research/replay-registry').createReplayRegistry({ generator, probability,
    getSpot: () => currentSpot, getState: () => currentState, strategy: config.strategy });
  const settlementReference = config.strategy.SETTLEMENT_AWARE
    ? new (require('./proxy-reference').ProxyReference)(history.prepare('SELECT * FROM markets ORDER BY close_time,ticker').all(), spotRows, { asset })
    : undefined;
