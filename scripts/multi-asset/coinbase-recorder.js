// Isolated research fork: BTC v3 policy remains untouched.
...
const { MAX_FUTURE_SKEW_MS } = require('../../src/research/coinbase-time');
class CoinbaseRecorder {
  constructor(filename, product, { Socket = WebSocket, clock = Date.now } = {}) {
    if (!['ETH-USD','SOL-USD','XRP-USD','DOGE-USD'].includes(product)) throw Error('Unsupported product');
    ...
    this.book = new CoinbaseBook(product); this.counts = {};
    this.Socket = Socket; this.clock = clock; this.retryMs = 1000; this.stopped = true;
  }
  record(message, received = this.clock()) {
    if (message.type !== 'ticker' || message.product_id !== this.product) return false;
    ...
    return this.insert.run(Math.floor(received / 1000) * 1000, event, received, price, bid, ask, `coinbase:${this.product}:ticker`, JSON.stringify(message)).changes > 0;
  }
  ...
  recordBook(m, received) {
    ...
    this.insertQuote.run(second, q.event, received, q.price, q.bid, q.ask, `coinbase:${this.product}:book-midpoint`, JSON.stringify(q));
  ...
  connect() {
    ...
    ws.on('open', () => {
      if (this.stopped) return;
      ws.send(JSON.stringify({ type: 'subscribe', product_ids: [this.product], channels: ['ticker', 'heartbeat', 'level2_batch'] }));
