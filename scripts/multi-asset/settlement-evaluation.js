...
function evaluateForecasts(history, spot, { asset = 'BTC', volatilityReturns = 15, indexReference, forwardAfter = null, forwardOnly = false } = {}) {
  ...
  const proxy = new ProxyReference(historyMarkets, spot, { asset, volatilityReturns }), model = new ProbabilityModel();
  ...
  for (const [marketIndex, m] of markets.entries()) {
    ...
    if (!m.ticker.startsWith(`KX${asset}15M-`) || raw.strike_type !== 'greater_or_equal' || closeTime - openTime !== 900000 ||
        !Number.isFinite(m.floor_strike) || m.floor_strike <= 0) { reject('unsupported_market'); continue; }
    const cs = candles.all(m.ticker);
    if(cs.length !== 15 || cs.some((c,i)=>c.end_period_ts*1000 !== openTime+(i+1)*60000)){reject('incomplete_market_candles');continue;}
    const qs = new Map(cs.map(c => [c.end_period_ts * 1000, c]));
    for (const minute of OFFSETS) {
      ...
      const sigma = minuteVolatility(spot.slice(Math.max(0, index - volatilityReturns), index + 1), 900, volatilityReturns);
